...
export class SolicitarconvenioComponent implements OnInit {
  ...
  getAllSolicitud() {
    this.solicitarService.getAllSolicitud().subscribe((data) => {
      this.solicitarLista = data;
      console.log(this.solicitarLista);
    });
  }
  saveSolicitud() {
    this.solicitar.idconvocatoria = {
      //idconvocatoria: Number(this.convocatoria.idconvocatoria),
      idconvocatoria: 1,
    } as Convocatoria;
    console.log('Hola Mundo');

    this.solicitarService.saveSolicitudes(this.solicitar).subscribe((data) => {
  ...
  getConvocatoria() {
    this.convocatoriaService.getConvocatoria().subscribe((response) => {
      console.table(response);
      this.convocatorias = response;
    });
  }
  delete(iddetalle_convo: number): void {
    this.solicitarService.delete(iddetalle_convo).subscribe((data) => {
      this.getAllSolicitud();
    });
  }
}
